...
#   1. a video folder, such as r"F:\...\南京东路"
#   2. one video file, such as r"F:\...\南京东路\hello kity.mp4"
#
# SIMPLE_MODE:
...
SIMPLE_VIDEO_PATH = r"F:\商业街光环境研究\商业街调研图片视频\视频汇总\南京东路\序列 01_1.mp4"
...
# Background ROI: use a dynamic screen-expansion ring based on the target
# screen size; if the ring has too few valid pixels, use the non-screen part of
# the frame instead.
SIMPLE_BACKGROUND_RING_MIN_PX = 31
SIMPLE_BACKGROUND_RING_MAX_PX = 120
SIMPLE_BACKGROUND_RING_RATIO = 0.5
...
SIMPLE_SCREEN_SIGNATURE_CUM_PROP = 0.65
SIMPLE_SCREEN_SIGNATURE_NEIGHBOR_RADIUS = 0
...
def expand_screen_to_background_ring(
    ...
    bbox_short = max(1, min(bbox_w, bbox_h))
    k = int(round(bbox_short * float(ratio)))
    k = max(int(lower_px), min(int(upper_px), k))
    if k % 2 == 0:
        k += 1
    ...
    for frame_idx in frame_indices:
        ...
        if target_area_ratio >= float(getattr(args, "local_bg_target_area_threshold", 0.30)):
            ...
            bg_mask = expand_screen_to_background_ring(
                screen_mask,
                exclude_mask,
                lower_px=int(getattr(args, "background_ring_min_px", 31)),
                upper_px=int(getattr(args, "background_ring_max_px", 120)),
                ratio=float(getattr(args, "background_ring_ratio", 0.25)),
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_extract.add_argument("--background-ring-min-px", type=int, default=31, help="Minimum dynamic screen-expansion kernel size for adjacent background ROI.")
    p_extract.add_argument("--background-ring-max-px", type=int, default=120, help="Maximum dynamic screen-expansion kernel size for adjacent background ROI.")
    p_extract.add_argument("--background-ring-ratio", type=float, default=0.25, help="Dynamic background-ring kernel size as a fraction of the target-screen shorter bounding-box side.")
...
def run_simple_pycharm_workflow() -> None:
    ...
    if SIMPLE_MODE in {"extract", "annotate_extract_screen"}:
        extract_args = argparse.Namespace(
            ...
            background_ring_min_px=int(SIMPLE_BACKGROUND_RING_MIN_PX),
            background_ring_max_px=int(SIMPLE_BACKGROUND_RING_MAX_PX),
            background_ring_ratio=float(SIMPLE_BACKGROUND_RING_RATIO),
